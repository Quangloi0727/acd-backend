...
export class FindByChannelsAndStatesCommandHandler
  implements ICommandHandler<FindByChannelsAndStatesCommand>
{
  constructor(
  ...
  async execute(body) {
    ...
    if (sortAsc && sortAsc.length) {
      sortAsc.forEach((element) => {
        if (element && element !== 'null') _sortQuery[element] = 1;
      });
    }
    ...
    if (filterText && filterText != '') {
      _query.senderName = {
        $regex: new RegExp(this.stringRegex(filterText), 'i'),
      };
    }

    if (
      cloudAgentId &&
      conversationStates.includes(ConversationState.CLOSE) == false
    ) {
      _query.$or = [];
      _query.$or.push(
        { participants: cloudAgentId },
